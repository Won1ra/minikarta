...
function init(){
    ...
    range = 25;
    baseTepmlateUrl = 'https://raw.githubusercontent.com/RonyRam/minikarta/master/';
    ...
    function updateloop() {
        ...
        var url = baseTepmlateUrl + "templates/data.json?" + new Date().getTime();
...
function checkNeededElems() {
    return window && document.getElementById('canvas') && document.getElementById('coordinatesNote');
};

function tryInit() {
    console.log('Try to init...');
    if (checkNeededElems()) init();
    else setTimeout(tryInit, 333);
};
...
if (window.loaded) tryInit();
    ...
    function inject() {
        ...
        s.appendChild(document.createTextNode(`(${tryInit})();${checkNeededElems};${init}`));
